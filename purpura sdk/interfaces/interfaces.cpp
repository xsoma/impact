...
namespace interfaces {
	...
	void interfaces::init() {
		...
		client = global_utils::get_interface <i_base_client_dll>((char*) "client.dll", (char*) "VClient018");
		std::cout << "[interface:] client: 0x" << client << "\n";
		client_ent_list = reinterpret_cast <i_client_entity_list*> (global_utils::find_interface("client.dll", "VClientEntityList"));
		...
		game_prediction = reinterpret_cast <c_prediction*> (global_utils::find_interface("client.dll", "VClientPrediction"));
		std::cout << "[interface:] game_prediction: 0x" << game_prediction << "\n";
		game_movement = reinterpret_cast <i_game_movement*> (global_utils::find_interface("client.dll", "GameMovement"));
		...
		movement_helper = **reinterpret_cast<i_move_helper***>((global_utils::get_signature("client.dll", "8B 0D ? ? ? ? 8B 46 08 68") + 0x2));
